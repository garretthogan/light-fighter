...
const SEPARATION_MIN_DIST = 2.0
const SEPARATION_STRENGTH = 0.6
const TUI_RED = 0xff4d4d

export class ArmoredSphereTarget {
  constructor(playerMesh, sphereTexture = null, spawnPosition = null, sphereInstanced = null, sphereIndex = null) {
  ...
  _buildCone() {
    ...
    this.cone.position.set(0, 0, 0.5)
  ...
  update(delta, playerPosition, boxTargets = [], otherMovingSpheres = []) {
    ...
    for (const box of boxTargets) {
      ...
      if (distToBox < SPHERE_RADIUS && distToBox > 1e-6) {
        ...
        const dotB = moveX * nx + moveZ * nz
        const reflX = moveX - 2 * dotB * nx
        const reflZ = moveZ - 2 * dotB * nz
